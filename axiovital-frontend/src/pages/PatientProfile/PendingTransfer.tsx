...
export const PendingTransfer: React.FC<PendingTransferProps> = ({ isOpen, onClose }) => {
  ...
  return (
    ...
    >
      ...
      >
        <span className="font-semibold text-gray-800">Pending Transfer</span>
      ...
      <div className="p-4 bg-white flex-1 space-y-4">
        <h3 className="text-blue-800 font-bold text-[13px] border-b pb-1">Register Pending Ward/Bed Transfer</h3>
        <div className="grid grid-cols-2 gap-4">
          <div className="flex flex-col gap-1">
            <label className="font-semibold">Target Ward:</label>
            <select className="border p-1 w-full bg-white"><option>Cardiac Care Unit</option></select>
          </div>
          <div className="flex flex-col gap-1">
            <label className="font-semibold">Estimated Bed Assignment Time:</label>
            <input type="text" className="border p-1 bg-white outline-none w-full" defaultValue="09-Sep-2023 16:00" />
          </div>
        </div>
      </div>
      <div className="flex justify-end gap-2 p-3 bg-gray-50 border-t">
        <button onClick={() => { alert('Pending transfer saved.'); onClose(); }} className="bg-[#2c5b96] hover:bg-[#1a4478] text-white px-6 py-1 rounded-sm font-bold">Save</button>
