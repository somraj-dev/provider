...
interface PendingDischargeProps {
  isOpen: boolean;
  onClose: () => void;
}

export const PendingDischarge: React.FC<PendingDischargeProps> = ({ isOpen, onClose }) => {
  ...
  return (
    ...
    >
      ...
      >
        <span className="font-semibold text-gray-800">Pending Discharge</span>
      ...
      <div className="p-4 bg-white flex-1 space-y-4">
        <h3 className="text-blue-800 font-bold text-[13px] border-b pb-1">Set Pending Discharge</h3>
        <div className="grid grid-cols-2 gap-4">
          <div className="flex flex-col gap-1">
            <label className="font-semibold">Expected Discharge Date/Time:</label>
            <input type="text" className="border p-1 bg-white outline-none w-full" defaultValue="10-Sep-2023 14:00" />
          </div>
          <div className="flex flex-col gap-1">
            <label className="font-semibold">Disposition:</label>
            <select className="border p-1 w-full bg-white"><option>Pending Medical Clearance</option></select>
          </div>
        </div>
      </div>
      <div className="flex justify-end gap-2 p-3 bg-gray-50 border-t">
        <button onClick={() => { alert('Pending discharge set.'); onClose(); }} className="bg-[#2c5b96] hover:bg-[#1a4478] text-white px-6 py-1 rounded-sm font-bold">Save</button>
